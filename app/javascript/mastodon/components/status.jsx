...
import New_status_action_bar from './new_status_action_bar';
...
class Status extends ImmutablePureComponent {
  ...
  render () {
    ...
    const accuracy = status.get('account').get('accuracy');
  
    const handleAccuracy = () => {
      if (accuracy && accuracy >= 80.0) return GoldBadge;
      if (accuracy && accuracy >= 60.0 && accuracy < 80.0) return SilverBadge;
      if (accuracy && accuracy >= 51.0 && accuracy < 60.0) return BronzeBadge;
      if (accuracy && accuracy >= 50.0 && accuracy < 51.0) return LeadBadge;
      if (accuracy && accuracy < 50.0) return BlackBadge;
    }
    ...
    if (pictureInPicture.get('inUse')) {
      ...
      media = (
        <Card
          ...
          compact
          sensitive={status.get('sensitive')}
          factChecked={status.get('fact_checked')}
          debunked={status.get('debunked')}
        />
      );
    }
      ...
      <HotKeys handlers={handlers}>
        <div className={classNames('status__wrapper', `status__wrapper-${status.get('visibility')}`, { 'status__wrapper-reply': !!status.get('in_reply_to_id'), unread, focusable: !this.props.muted })} tabIndex={this.props.muted ? null : 0} data-featured={featured ? 'true' : null} aria-label={textForScreenReader(intl, status, rebloggedByText)} ref={this.handleRef} data-nosnippet={status.getIn(['account', 'noindex'], true) || undefined}>
          ...
          <div className={classNames('status', `status-${status.get('visibility')}`, { 'status-reply': !!status.get('in_reply_to_id'), 'status--in-thread': !!rootId, 'status--first-in-thread': previousId && (!connectUp || connectToRoot), muted: this.props.muted })} data-id={status.get('id')}>
            ...
            {accuracy !== undefined && <img src={handleAccuracy()} alt={handleAccuracy()} style={{ position: 'absolute', top: '37px', left: '37px', width: '7%', zIndex: 1 }} />}
              ...
              <a onClick={this.handleAccountClick} href={`/@${status.getIn(['account', 'acct'])}`} title={status.getIn(['account', 'acct'])} className='status__display-name' target='_blank' rel='noopener noreferrer'>
                <div className='status__avatar'>
                  {/* {true && <img src={trustedIcon} style={{ position: 'absolute', zIndex: '1', left: '37px', top: '37px' }} />} */}
                  {statusAvatar}
                </div>

                <DisplayName account={status.get('account')} accuracy={accuracy} />
              </a>
            </div>
            ...
            {status.get('fact_checked') && (
              <div style={{ display: 'flex' }}>
                <img src={trustIcon} alt={trustIcon} style={{ marginRight: '5px', filter: 'invert(31%) sepia(86%) saturate(1945%) hue-rotate(86deg) brightness(93%) contrast(102%)'}} />
                <p style={{ margin: '10px 0px', color: '#05b108' }}>
            ...
            {status.get('debunked') && (
              <div style={{ display: 'flex' }}>
                <img src={warningIcon} alt={warningIcon} style={{ marginRight: '5px', filter: 'brightness(0) saturate(100%) invert(78%) sepia(77%) saturate(538%) hue-rotate(357deg) brightness(97%) contrast(95%)'}} />
                <p style={{ margin: '10px 0px', color: '#f1cf0b' }}>
                This content has been Debunked.
                </p>
              </div>
            )}
            <New_status_action_bar scrollKey={scrollKey} status={status} account={account} onFilter={matchedFilters ? this.handleFilterClick : null} {...other} />
