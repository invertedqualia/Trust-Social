...
class StatusRelationshipsPresenter
  ...
  attr_reader :reblogs_map, :favourites_map, :mutes_map, :pins_map, :trusts_map, :distrusts_map,
              :bookmarks_map, :likes_map, :dislikes_map, :filters_map, :attributes_map
  ...
  def initialize(statuses, current_account_id = nil, **options)
    if current_account_id.nil?
      ...
      @likes_map       = Status.likes_map(status_ids, current_account_id).merge(options[:likes_map] || {})
      @dislikes_map    = Status.dislikes_map(status_ids, current_account_id).merge(options[:dislikes_map] || {})
      @mutes_map       = Status.mutes_map(conversation_ids, current_account_id).merge(options[:mutes_map] || {})
      @pins_map        = Status.pins_map(pinnable_status_ids, current_account_id).merge(options[:pins_map] || {})
      @trusts_map      = Status.trusts_map(status_ids, current_account_id).merge(options[:trusts_map] || {})
      @distrusts_map      = Status.distrusts_map(status_ids, current_account_id).merge(options[:distrusts_map] || {})
