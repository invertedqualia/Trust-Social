...
class REST::StatusSerializer < ActiveModel::Serializer
  ...
  attributes :id, :created_at, :in_reply_to_id, :in_reply_to_account_id,
             ...
             :favourites_count, :edited_at, :fact_checked, :debunked

  attribute :favourited, if: :current_user?
  attribute :trusted, if: :current_user?
  attribute :distrusted, if: :current_user?
  attribute :reblogged, if: :current_user?
  attribute :muted, if: :current_user?
  attribute :bookmarked, if: :current_user?
  attribute :liked, if: :current_user?
  attribute :disliked, if: :current_user?
  ...
  def trusted
    if relationships
      relationships.trusts_map[object.id] || false
    else
      current_user.account.trusted?(object)
    end
  end

  def distrusted
    if relationships
      relationships.distrusts_map[object.id] || false
    else
      current_user.account.distrusted?(object)
    end
  end
  ...
  def liked
    if relationships
      relationships.likes_map[object.id] || false
    else
      current_user.account.liked?(object)
    end
  end

  def disliked
    if relationships
      relationships.dislikes_map[object.id] || false
    else
      current_user.account.disliked?(object)
    end
  end
