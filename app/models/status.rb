...
class Status < ApplicationRecord
  ...
  has_many :trusts, inverse_of: :status, dependent: :destroy
  has_many :distrusts, inverse_of: :status, dependent: :destroy
  has_many :favourites, inverse_of: :status, dependent: :destroy
  has_many :bookmarks, inverse_of: :status, dependent: :destroy
  has_many :likes, inverse_of: :status, dependent: :destroy
  ...
  has_many :local_trusted, -> { merge(Account.local) }, through: :trusts, source: :account
  has_many :local_distrusted, -> { merge(Account.local) }, through: :distrusts, source: :account
  has_many :local_liked, -> { merge(Account.local) }, through: :likes, source: :account
  has_many :local_disliked, -> { merge(Account.local) }, through: :dislikes, source: :account
  ...
  class << self
    ...
    def trusts_map(status_ids, account_id)
      Trust.select('status_id').where(status_id: status_ids).where(account_id: account_id).map { |f| [f.status_id, true] }.to_h
    end

    def distrusts_map(status_ids, account_id)
      Distrust.select('status_id').where(status_id: status_ids).where(account_id: account_id).map { |f| [f.status_id, true] }.to_h
    end
    ...
    def likes_map(status_ids, account_id)
      Like.select('status_id').where(status_id: status_ids).where(account_id: account_id).map { |f| [f.status_id, true] }.to_h
    end

    def dislikes_map(status_ids, account_id)
      Dislike.select('status_id').where(status_id: status_ids).where(account_id: account_id).map { |f| [f.status_id, true] }.to_h
    end
