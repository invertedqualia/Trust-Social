...
module AccountInteractions
  ...
  def trusted?(status)
    status.proper.trusts.where(account: self).exists?
  end

  def distrusted?(status)
    status.proper.distrusts.where(account: self).exists?
  end
  ...
  def liked?(status)
    status.proper.likes.where(account: self).exists?
  end
